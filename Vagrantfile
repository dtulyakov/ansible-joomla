...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  ## Configure joomla projects
  vagrant_config['projectnames'].each do |project|
      config.vm.provision "ansible_local" do |ansible|
          ansible.playbook = "ansible/playbook.yml"
          ansible.start_at_task = "Download joomla Installer"
          ansible.sudo = true
          ansible.verbose = ""
          ansible.extra_vars = {
              servername: vagrant_config['servername'],
              projectname: project,
              testing_mode: 0, 
              joomla_version: vagrant_config['joomla_version'],
              ansible_host: vagrant_config['private_ip']
          }
      end
  end

  ## Ensure Apache and Nginx restart after vagrant up
  config.vm.provision "shell", inline: "service apache2 restart && service nginx restart", run: "always"
